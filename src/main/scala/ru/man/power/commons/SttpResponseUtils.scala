...
object SttpResponseUtils {
  ...
  def unwrapResponseOrError[F[_] : Applicative, T: Decoder]: ResponseAs[F[Either[sttp.client3.DeserializationException[io.circe.Error],T]], Any] = asJsonAlways[T].map(x => Applicative[F].pure(x))


  def unwrapResponseOrError2[F[_] : Applicative, E: Decoder, T: Decoder]: ResponseAs[F[Either[ResponseException[E, io.circe.Error], T]], Any] = {
